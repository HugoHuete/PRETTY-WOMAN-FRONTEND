...
describe('AppShell', () => {
  it('opens the tablet menu and closes it with Escape', async () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Menú de navegación' })).toBeVisible();
    expect(
      within(screen.getByRole('dialog', { name: 'Menú de navegación' })).getByRole(
        'navigation',
  ...
  it('marks the current route and omits admin links for Employee', () => {
    ...
    expect(screen.getByRole('img', { name: 'Pretty Woman Boutique' })).toBeInTheDocument();
    expect(screen.getAllByText('María Pérez')).toHaveLength(1);
    expect(screen.getAllByText('Vendedora')).toHaveLength(1);
  ...
  it('keeps sign out in the sidebar profile menu', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Abrir perfil de María Pérez' }));
